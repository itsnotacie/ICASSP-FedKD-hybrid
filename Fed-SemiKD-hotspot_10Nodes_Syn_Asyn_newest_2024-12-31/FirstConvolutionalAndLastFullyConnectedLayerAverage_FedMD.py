...
class FirstConvolutionalAndLastFullyConnectedLayerAverage_FedMD():
    ...
    def collaborative_training(self):  # 开始联邦蒸馏学习，异步从这里改造
        ...
        while True:
            ...
            # 初始化累加变量，最后一层全连接层参数
            last_fc_weights_sum = None
            last_fc_bias_sum = None
            ...
            for index, d in enumerate(self.collaborative_parties):
                ...
                if index in index_random:
                    ...
                    # print("第",index,"个模型的形状是:")
                    # 打印模型摘要
                    # d["model_logits"].summary()
                    ...
                    # 提取最后一层全连接层的权重和偏置
                    last_fc_weights = weights[-2]
                    last_fc_bias = weights[-1]

                    # 累加最后一层全连接层的权重
                    if last_fc_weights_sum is None:
                        last_fc_weights_sum = last_fc_weights
                    else:
                        last_fc_weights_sum += last_fc_weights

                    # 累加最后一层全连接层的偏置项
                    if last_fc_bias_sum is None:
                        last_fc_bias_sum = last_fc_bias
                    else:
                        last_fc_bias_sum += last_fc_bias
            ...
            # 计算平均值
            last_fc_weights_average = last_fc_weights_sum / asynchronousNoteNumber
            last_fc_bias_average = last_fc_bias_sum / asynchronousNoteNumber
            ...
            # 输出累加后的卷积层权重的形状
            print("平均全连接层权重形状:", last_fc_weights_average.shape)
            print("平均全连接层偏置形状:", last_fc_bias_average.shape)
            ...
            # 遍历每个模型
            for d in self.collaborative_parties:
                # 提取模型的权重，将卷积层平均和最后一层全连接池层平均赋值给模型
                d["model_weights"][0] = conv1_weights_average
                d["model_weights"][1] = conv1_bias_average
                d["model_weights"][-2] = last_fc_weights_average
                d["model_weights"][-1] = last_fc_bias_average
                # 设置模型的新权重
