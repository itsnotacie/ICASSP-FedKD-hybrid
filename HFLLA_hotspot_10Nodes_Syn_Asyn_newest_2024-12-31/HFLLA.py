            ...
            for index, d in enumerate(self.collaborative_parties):
                ...
                for index, d in enumerate(self.collaborative_parties):
                    if index in index_random:
                        ...
                        # 获取当前的模型权重
                        full_weights = d["model_weights"]

                        # 设置模型的所有权重
                        d["model_classifier"].set_weights(full_weights)

                        # 锁定前面层的权重，只训练最后两层
                        for layer in d["model_classifier"].layers[:-2]:
                        ...
                        # 只更新最后两层的权重
                        updated_weights = d["model_classifier"].get_weights()
                        d["model_weights"][-2:] = updated_weights[-2:]

                        print("model {0} done private training with last two layers updated.\n".format(index))
                ...
                for index, d in enumerate(self.collaborative_parties):
                    ...
                    if index >= self.N_parties // 2:
                        ...
                        TPR = sum(self.private_test_data_FAB1["y"] + y_pred == 2) / sum(
